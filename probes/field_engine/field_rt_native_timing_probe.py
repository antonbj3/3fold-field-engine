...
def event(label):
    with (OUT/'field_rt_native_timing_events.jsonl').open('a') as f:
...
def run():
    ...
    from field_rt_native_edt_probe import edt as parallel_edt
    ...
    (OUT/'field_rt_native_timing_v1.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(OUT/'field_rt_native_timing_v1_arrays.npz',**arrays)
...
def main():
    log=OUT/'field_rt_native_timing_events.jsonl'
