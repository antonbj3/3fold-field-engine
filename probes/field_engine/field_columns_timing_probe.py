...
from mesh_field_columns_v1 import PreparedMeshField, surface_raster_and_flood_rt
...
def main():
    ...
    if not idle:
        ...
        (root/'reports/columns_timing.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report));return 2
    ...
    for pitch in (2.,1.,.5):
        ...
        try:
            for leg in range(2):
                ...
                hashes={};differences={}
                for method,result in (('cpu',ref),('one',one),('prepared',prepared)):
                ...
                rows.append(dict(pitch=pitch,leg=leg,shape=list(ref[1]),cpu_ms=cpu_ms,one_shot_ms=one_ms,prepared_ms=prepared_ms,setup_ms=setup_ms,hashes=hashes,differences=differences))
        finally:field.close()
    ...
    (root/'reports/columns_timing.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(root/'reports/columns_timing_arrays.npz',**arrays)
