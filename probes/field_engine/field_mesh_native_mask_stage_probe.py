...
def main():
    ...
    for name,(mesh,pitch,lo,closed) in cases().items():
        ...
        legs=[]
        with PreparedMeshField(v,f,pitch,lo,winding_library=os.environ['RT_COLUMNS_LIBRARY'],ptx=os.environ['RT_COLUMNS_PTX'],edt_library=os.environ['NATIVE_EDT_LARGE_LIBRARY'],column_library=os.environ['COLUMN_MASK_LIBRARY']) as field:
            for leg in range(2):
    ...
    (folder/'mesh_native_mask_stage.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(folder/'mesh_native_mask_stage_arrays.npz',**arrays)
