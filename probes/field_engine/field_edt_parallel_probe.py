...
def event(label):
    with (OUT/'field_edt_parallel_events.jsonl').open('a') as f:
...
def run():
    ...
    (OUT/'field_edt_parallel_v1.json').write_text(json.dumps(report, indent=2)+'\n')
    np.savez_compressed(OUT/'field_edt_parallel_v1_arrays.npz', **arrays)
    print(json.dumps(dict(gates=gates, rows=len(rows))))
    return 0 if all(gates.values()) else 1


def main():
    if (OUT/'field_edt_parallel_events.jsonl').exists():
