"""Complete native-read/pack CSG versus prior CSG and prepared B-rep."""
...
from field_csg_native_v1 import evaluate as previous_evaluate
from field_csg_packed_v1 import evaluate as packed_evaluate
...
def main():
    ...
    def python_evaluate(fields,program):
        return previous_evaluate(fields,program,reader=reader)
    def native_evaluate(fields,program,*,reader):
        return packed_evaluate(fields,program,reader=reader,pack_library=pack_library)
    rows,arrays=[],{}
    ...
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,invalid=invalid,
                source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),pack_source_sha256=hashlib.sha256(pack_source.read_bytes()).hexdigest(),
                scope='python timing key means native tile reads plus previous Python pack; native means native reads and native pack. NumPy CSG arithmetic unchanged. B-rep booleans use prepared primitives. Eight reversed-order comparisons, no compilation/preparation/extraction in operation timing. Ordinary concurrent CPU load. Full saved field equality transfers prior fixture accuracy only.')
    (ROOT/'reports/field_csg_packed.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_csg_packed_arrays.npz',**arrays)
