...
from mesh_field_compact_v1 import PreparedMeshField, surface_raster_and_flood_rt
from mesh_field_columns_v1 import PreparedMeshField as FullPrepared
...
def main():
    ...
    if not idle:
        ...
        (root/'reports/compact_timing.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report));return 2
    ...
    kwargs=dict(winding_library=os.environ['RT_COLUMNS_LIBRARY'],ptx=os.environ['RT_COLUMNS_PTX'],edt_library=os.environ['NATIVE_EDT_LARGE_LIBRARY'])
    rows=[];arrays={}
    def timed(fn):
    ...
    for pitch in (2.,1.,.5):
        ...
        full_kwargs=dict(kwargs);full_kwargs['winding_library']=os.environ['RT_FULL_LIBRARY']
    ...
    gates['compact_L_faster_than_full']=all(r['prepared_ms']<r['full_prepared_ms'] for r in rows if r['pitch']==.5)
    ...
    (root/'reports/compact_timing.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(root/'reports/compact_timing_arrays.npz',**arrays)
