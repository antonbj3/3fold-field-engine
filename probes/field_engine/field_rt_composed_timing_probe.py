"""Measure fixed64 RT plus parallel EDT composition with runtime setup charged."""
...
def event(label):
    with (OUT/'field_rt_composed_timing_events.jsonl').open('a') as f:
...
def run():
    ...
    guard();idle()
    event('warp_setup_start');start=time.perf_counter()
    import warp as wp
    from field_rt_parallel_edt_probe import edt as parallel_edt
    wp.init()
    runtime_setup=(time.perf_counter()-start)*1000;event('warp_setup_end')
    for leg in range(2):
        for ref in refs[::3]:
    ...
    report=dict(rows=rows,gates=gates,timing_repeat_identity_claim=False,runtime_setup_ms=runtime_setup,scope='Prepared fixed mesh and three ray grids; accounted timings exclude audit/hash I/O and offline fixture preparation.')
    (OUT/'field_rt_composed_timing_v1.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(OUT/'field_rt_composed_timing_v1_arrays.npz',**arrays)
...
def main():
    log=OUT/'field_rt_composed_timing_events.jsonl'
