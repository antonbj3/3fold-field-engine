"""Full CPU equality and unchanged 2 ms gate for borrowed field delivery."""
...
def main():
    libraries = dict(fused_library=os.environ['MESH_FUSED_SHARED_LIBRARY'],
    ...
    sources = ('mesh_field_shared_service_v1.py', 'mesh_fused_into_v1.py', 'mesh_fused_shared_v1/field.cu',
               'mesh_fused_native_v1/field.cu', 'field_shared_gpu_probe.py')
    report = dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL', gates=gates,
                  ...
                  scope='Fourteen fresh spawned workers in two fixture orders. Setup includes spawn/import/prepare/attach/pin. Every query recomputes full fields into shared memory and returns borrowed read-only views. First query separate; last four are warm. Explicit copy cost is excluded from borrowed latency and reported separately; snapshots retained for every query. No owned-output service acceptance claim.')
    (ROOT/'reports/field_shared_gpu.json').write_text(json.dumps(report, indent=2) + '\n')
    np.savez_compressed(ROOT/'reports/field_shared_gpu_arrays.npz', **captures)
