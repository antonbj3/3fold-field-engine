...
def event(label):
    with (OUT/'field_rt_api_contiguous_events.jsonl').open('a') as f:
...
def run():
    ...
    data={key:np.ascontiguousarray(data[key]) for key in data.files}
    ...
    (OUT/'field_rt_api_contiguous_v1.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(OUT/'field_rt_api_contiguous_v1_arrays.npz',**arrays)
    print(json.dumps(report));return 0 if all(gates.values()) else 1

def main():
    log=OUT/'field_rt_api_contiguous_events.jsonl'
