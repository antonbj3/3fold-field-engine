"""Certify persistent RT and native integer EDT coexistence with frozen outputs."""
...
def event(label):
    with (OUT/'field_rt_native_edt_events.jsonl').open('a') as f:
...
def edt(mask,h):
    global _library
    if _library is None:
        _library=ct.CDLL(os.environ['EDT_NATIVE_LIBRARY'])
        _library.edt_squared.argtypes=[ct.c_void_p,ct.c_int,ct.c_int,ct.c_int,ct.c_void_p]
        _library.edt_squared.restype=ct.c_int
    squares=[]
    for binary in (~mask,mask):
        source=np.ascontiguousarray(binary,dtype=np.uint8)
        output=np.empty(source.shape,dtype=np.int32)
        code=_library.edt_squared(source.ctypes.data,*source.shape,output.ctypes.data)
        if code:raise RuntimeError(f'Native EDT status {code}')
...
def run():
    ...
    (OUT/'field_rt_native_edt_v1.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(OUT/'field_rt_native_edt_v1_arrays.npz',**arrays)
    print(json.dumps(report));return 0 if all(gates.values()) else 1

def main():
    log=OUT/'field_rt_native_edt_events.jsonl'
