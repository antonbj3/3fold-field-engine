...
def main():
        ...
        for pitch in ((2.,1.,.5) if leg==0 else (.5,1.,2.)):
            ...
            try:
                for name,library in libraries.items():
                    handles[name],setup[name]=timed(lambda:ColumnsHandle(library,os.environ['RT_INTERVAL_PTX'] if name=='filtered' else ptx,corners,anchor,len(xy)))
    ...
    for leg in range(2):
        for count in (32,33):
            ...
            h=ColumnsHandle(libraries['filtered'],os.environ['RT_INTERVAL_PTX'],corners,np.zeros(3),1)
            try:
                def foreign():
                    try:h.query(xy)
    ...
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,
                source_sha256=hashlib.sha256((ROOT/'src/field_engine/rt_columns_interval_v1/program.cu').read_bytes()).hexdigest(),
    ...
    (ROOT/'reports/field_rt_interval_hits.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_rt_interval_hits_arrays.npz',**arrays)
