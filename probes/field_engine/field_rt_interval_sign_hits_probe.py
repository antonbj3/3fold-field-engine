...
def main():
    ...
    libraries=dict(strided=os.environ['RT_COLUMNS_LIBRARY'],filtered=os.environ['RT_COLUMNS_LIBRARY'])
        ...
        for pitch in ((2.,1.,.5) if leg==0 else (.5,1.,2.)):
            ...
            try:
                for name,library in libraries.items():
                    handles[name],setup[name]=timed(lambda:ColumnsHandle(library,os.environ['RT_INTERVAL_SIGN_PTX'] if name=='filtered' else ptx,corners,anchor,len(xy)))
    ...
    for leg in range(2):
        for count in (32,33):
            ...
            h=ColumnsHandle(libraries['filtered'],os.environ['RT_INTERVAL_SIGN_PTX'],corners,np.zeros(3),1)
            try:
                def foreign():
                    try:h.query(xy)
    ...
    gates=dict(coverage=len(rows)==24,exact=all(r['exact'] for r in rows),repeat=all(len({json.dumps(r['hashes']) for r in rows if r['pitch']==p})==1 for p in (2.,1.,.5)),
               capacity_lifecycle=len(controls)==28 and all(controls))
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,
                source_sha256=hashlib.sha256((ROOT/'src/field_engine/rt_columns_interval_sign_v1/program.cu').read_bytes()).hexdigest(),
                scope='Same compact host ABI and native RT geometry. Candidate PTX adds conservative intervals before unchanged double triangle tests. All CPU/native hits and capacity controls required; timings retained without a speed acceptance claim. Full service measured separately.')
    (ROOT/'reports/field_rt_interval_sign_hits.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_rt_interval_sign_hits_arrays.npz',**arrays)
