...
import numpy as np
from rt_interval_portable_v1 import fixtures, bounds, frozen_edges, subtract
...
def main():
    ...
    report = dict(status='VERIFIED-FRESH' if all(controls.values()) else 'OWN-GATE-FAIL', gates=controls,
                  ...
                  source_sha256=hashlib.sha256((ROOT/'src/field_engine/rt_interval_portable_v1.py').read_bytes()).hexdigest(),
    ...
    (ROOT/'reports/field_rt_interval_portable.json').write_text(json.dumps(report, indent=2) + '\n')
    np.savez_compressed(ROOT/'reports/field_rt_interval_portable_arrays.npz', triangles=triangles, queries=queries,
