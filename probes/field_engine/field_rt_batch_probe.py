...
def event(label, case=None, leg=None):
    ...
    EVENTS.append(row)
    with (ROOT/'artifacts'/'field_rt_batch_events.jsonl').open('a') as output:
...
def main():
        ...
        for leg in range(2):
            for name,mesh in meshes.items():
                ...
                for repeat in range(64):
                    ...
                    cpu_sum+=(time.perf_counter()-t)*1000
                    if first_cpu is None: first_cpu=reference.copy()
                    else: assert np.array_equal(first_cpu,reference)
                cpu_total=cpu_sum/64
                ...
                for repeat in range(64):
                    ...
                    if first_sd is None: first_sd=sd.copy()
                    else: assert np.array_equal(first_sd,sd)
                batch_total=(time.perf_counter()-start)*1000
                total=batch_total/64
                event('native_total_end',name,leg)
                guard()
                row=dict(batch_repetitions=64,native_batch_total_ms=batch_total,native_phases_ms=json.loads(result.stdout),case=name,leg=leg,idle_utilization_percent=utilization,cpu_total_ms=cpu_total,
    ...
    gates=dict(phases_finite_nonnegative=all(np.isfinite(v) and v>=0 for r in rows for v in r['native_phases_ms'].values()),full_repeat=all(rows[i]['counts_sha256']==rows[i+4]['counts_sha256'] and rows[i]['distance_sha256']==rows[i+4]['distance_sha256'] for i in range(4)),
    ...
    out=ROOT/'artifacts'
    (out/'field_rt_batch_v1.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(out/'field_rt_batch_v1_arrays.npz',**outputs)
    print(json.dumps(report)); return 0 if all(gates.values()) else 1

def locked_main():
    destination=ROOT/'artifacts'/'field_rt_batch_events.jsonl'
