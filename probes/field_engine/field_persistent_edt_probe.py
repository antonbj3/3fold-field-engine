...
def main():
    ...
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,
                source_sha256=hashlib.sha256((ROOT/'src/field_engine/edt_persistent_pair_v1/edt.cu').read_bytes()).hexdigest(),
    ...
    (ROOT/'reports/field_persistent_edt.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_persistent_edt_arrays.npz',**arrays)
