...
def event(label, case=None, leg=None):
    ...
    EVENTS.append(row)
    with (ROOT/'artifacts'/'field_rt_native_phase_events.jsonl').open('a') as output:
...
def main():
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for leg in range(2):
            for name,mesh in meshes.items():
                ...
                guard()
                row=dict(native_phases_ms=json.loads(result.stdout),case=name,leg=leg,idle_utilization_percent=utilization,cpu_total_ms=cpu_total,
    ...
    out=ROOT/'artifacts'
    (out/'field_rt_native_phase_v1.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(out/'field_rt_native_phase_v1_arrays.npz',**outputs)
    print(json.dumps(report)); return 0 if all(gates.values()) else 1

def locked_main():
    destination=ROOT/'artifacts'/'field_rt_native_phase_events.jsonl'
