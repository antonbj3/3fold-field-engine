"""Measure a separate local-frame adapter against the frozen world-coordinate path."""
...
def main():
    ...
    overlap=first.copy(); overlap.apply_translation([1,0.5,0.25])
    nested=trimesh.creation.box(extents=[2,2,2])
    offset=first.copy(); offset.apply_translation([0.13,-0.21,0.37])
    reverse=first.copy(); reverse.faces=reverse.faces[:,::-1]
    cases.update(box=first,offset=offset,reversed=reverse,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for name, mesh in cases.items():
            pitch = 0.25
            origin = np.array([-4.,-4.,-4.]) if name in ('box','offset','reversed','overlap','nested') else np.asarray(mesh.bounds).min(axis=0)-1.0
            ...
            for leg in range(2):
                ...
                if result.returncode:
                    ...
                    (ROOT/'artifacts'/'field_rt_winding_local_v1_failure.json').write_text(json.dumps(failure, indent=2)+'\n')
    ...
    (folder/'field_rt_winding_local_v1.json').write_text(json.dumps(report, indent=2)+'\n')
    np.savez_compressed(folder/'field_rt_winding_local_v1_arrays.npz', **arrays)
