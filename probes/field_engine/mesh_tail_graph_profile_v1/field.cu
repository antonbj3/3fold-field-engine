// Observer-only GPU events around the same trace and captured CUDA tail.
...
#include "../../../src/field_engine/mesh_fused_shared_v1/field.cu"
#undef fused_create
#undef fused_query
#undef fused_destroy
#undef fused_pin
...
struct GraphField {
    ...
    int* flags=nullptr;bool failed=false;cudaEvent_t events[5]{};bool measured=false;
};
extern "C" int fused_destroy(void** handle) noexcept {
    ...
    for(auto event:g->events)if(event&&cudaEventDestroy(event)!=cudaSuccess)status=2;
...
extern "C" int fused_create(const char* path,const double* corners,uint32_t triangles,const double* anchor,const double* xy,int nx,int ny,int nz,double pitch,const double* zs,void** handle) noexcept {
    ...
    try{
        ...
        for(auto& event:g->events)check(cudaEventCreate(&event));
        ...
        check(cudaEventRecordWithFlags(g->events[2],s.stream,cudaEventRecordExternal));
        ...
        check(cudaEventRecordWithFlags(g->events[3],s.stream,cudaEventRecordExternal));
...
extern "C" int fused_query(void* handle,uint8_t* solid,uint8_t* surface,float* distance) noexcept {
    ...
    try{
        check(cudaEventRecord(g.events[0],s.stream));
        check(optixLaunch(s.pipeline,s.stream,s.params_data,sizeof(WindingParams),&s.sbt,f.columns,1,1));
        check(cudaEventRecord(g.events[1],s.stream));
        ...
        check(cudaEventRecord(g.events[4],s.stream));
        check(cudaStreamSynchronize(s.stream));g.measured=true;
...
extern "C" int fused_profile(void* handle,float* output) noexcept {
    if(!handle||!output)return 1;auto& g=*static_cast<GraphField*>(handle);
    auto& f=*static_cast<Fused*>(g.base);auto& s=*static_cast<State*>(f.rt);
    if(s.owner!=std::this_thread::get_id())return 3;if(!g.measured||g.failed)return 2;
    for(int i=0;i<4;++i)if(cudaEventElapsedTime(output+i,g.events[i],g.events[i+1])!=cudaSuccess)return 2;
    return 0;
}
