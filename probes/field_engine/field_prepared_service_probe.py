"""Independent full field snapshots, exact CPU references and unchanged 2 ms service gate."""
...
def main():
    libraries = dict(fused_library=os.environ['MESH_FUSED_PREPARED_LIBRARY'],
                     winding_library=os.environ['RT_COLUMNS_LIBRARY'], ptx=os.environ['RT_PREPARED_PTX'],
        ...
        for name in (list(fixtures) if leg == 0 else list(fixtures)[::-1]):
            ...
            with MeshFieldService(**libraries) as service:
                ...
                for repeat in range(5):
                    (result, worker_ms), elapsed = timed(service.query)
                    snapshot = tuple(np.asarray(x) for x in result)
                    exact &= same(snapshot, expected)
                    timings.append(elapsed); workers.append(worker_ms)
                ...
                # Every prior output remains equal after subsequent recomputation.
                exact &= all(same(tuple(captures[f'{name}_{leg}_{repeat}_{i}'] for i in range(5)), expected) for repeat in range(5))
                rows.append(dict(case=name, leg=leg, setup_ms=setup_ms, pinned=service.metadata['pinned'],
                                 query_ms=timings, worker_ms=workers,
                ...
                controls.append(same(replacement, ref))
                try:
    ...
    for count in (32, 33):
        ...
        with MeshFieldService(**libraries) as service:
            ...
            if count == 32:
                ...
                controls.append(same(result, baseline.surface_raster_and_flood(vertices, faces, 1., origin)))
                for i, a in enumerate(result):
                    captures[f'capacity_{i}'] = np.asarray(a)
            else:
                for _ in range(2):
                    try:
                        service.query()
    ...
    sources = ('mesh_field_shared_service_v1.py', 'mesh_fused_into_v1.py', 'mesh_fused_shared_v1/field.cu', 'mesh_fused_tail_graph_v1/field.cu', 'mesh_field_owned_service_v1.py',
               'rt_columns_prepared_v1/host_bounds.hpp', 'rt_columns_prepared_v1/filter.cuh', 'rt_columns_prepared_v1/program.cu', 'rt_columns_prepared_v1/params.h', 'rt_columns_prepared_v1/api.cpp', 'mesh_fused_prepared_v1/field.cu', 'mesh_fused_prepared_shared_v1/field.cu', 'mesh_fused_prepared_graph_v1/field.cu', 'mesh_fused_native_v1/field.cu', 'field_prepared_service_probe.py')
    report = dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL', gates=gates,
                  ...
                  scope='Fourteen fresh spawned workers in two fixture orders. Setup includes spawn/import/prepare/graph capture/attach/pin. Every query retraces geometry, recomputes full fields and returns independent writable snapshots; all copy work is included in query time. First query separate; last four are warm. All snapshots retained and checked after subsequent queries. No samples discarded and no tolerance changes.')
    (ROOT/'reports/field_prepared_service.json').write_text(json.dumps(report, indent=2) + '\n')
    np.savez_compressed(ROOT/'reports/field_prepared_service_arrays.npz', **captures)
