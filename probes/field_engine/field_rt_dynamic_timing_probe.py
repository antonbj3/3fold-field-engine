...
def event(label):
    with (OUT/'field_rt_dynamic_timing_events.jsonl').open('a') as f:
...
def run():
        ...
        for ref in refs[::3]:
            ...
            try:
                for call in range(64):
                    ...
                    cpu_sd=edt(mask,h)
                    surface=mask & ~ndimage.binary_erosion(mask,ndimage.generate_binary_structure(3,1))
                    ...
                    inside=counts!=0;sd=edt(inside,h)
                    ...
                    exact_ok &= np.array_equal(inside,mask) and np.array_equal(sd,cpu_sd) and np.array_equal(sd,data[f'{name}_{phase}_distance'])
            ...
            accounted=(setup+close+sum(query_times)+sum(edt_times))/64
            rows.append(dict(case=name,leg=leg,queries=64,setup_ms=setup,close_ms=close,cpu_mean_ms=cpu,wrapper_query_mean_ms=query,edt_surface_mean_ms=distance,
    ...
    report=dict(rows=rows,gates=gates,timing_repeat_identity_claim=False,scope='Prepared fixed mesh and three ray grids; accounted timings exclude audit/hash I/O and offline fixture preparation.')
    (OUT/'field_rt_dynamic_timing_v1.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(OUT/'field_rt_dynamic_timing_v1_arrays.npz',**arrays)
...
def main():
    log=OUT/'field_rt_dynamic_timing_events.jsonl'
