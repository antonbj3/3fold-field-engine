"""Compare an external signed RT winding candidate to the frozen field contract."""
...
def main():
    ...
    shifted = first.copy(); shifted.apply_translation([0.13, -0.21, 0.37])
    reverse = first.copy(); reverse.faces = reverse.faces[:, ::-1]
    cases = dict(box=first, offset=shifted, reversed=reverse,
        ...
        for name, mesh in cases.items():
            ...
            for leg in range(2):
                ...
                if result.returncode == 0:
                    ...
                    item.update(winding_sha256=digest(counts), distance_sha256=digest(sd),
                ...
                if result.returncode:
                    ...
                    (ROOT/'artifacts'/'field_rt_winding_v1_failure.json').write_text(json.dumps(failure, indent=2)+'\n')
    ...
    (folder/'field_rt_winding_v1.json').write_text(json.dumps(report, indent=2)+'\n')
    np.savez_compressed(folder/'field_rt_winding_v1_arrays.npz', **arrays)
