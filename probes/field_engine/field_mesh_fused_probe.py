"""Exact full device-resident field chain versus CPU and the two-hit stage."""
...
from mesh_field_two_hit_v1 import PreparedMeshField as ReferenceField
...
def main():
    ...
    candidate=dict(common,fused_library=os.environ['MESH_FUSED_LIBRARY'])
    reference=dict(common,pair_library=os.environ['NATIVE_EDT_PAIR_LIBRARY'])
    ...
    for name,(mesh,pitch,origin,closed) in cases().items():
        ...
        with ReferenceField(vertices,faces,pitch,origin,**reference) as old,PreparedMeshField(vertices,faces,pitch,origin,**candidate) as new:
            ...
            for leg in range(2):
                actual,times={},{}
                for mode in (('two_hit','fused') if leg==0 else ('fused','two_hit')):
                    actual[mode],times[mode]=timed((old if mode=='two_hit' else new).evaluate)
                rows.append(dict(case=name,leg=leg,times_ms=times,exact=same(actual['two_hit'],actual['fused']) and same(actual['fused'],expected),
                                 hashes=[hashlib.sha256(np.asarray(a).tobytes()).hexdigest() for a in actual['fused']]))
                for i,a in enumerate(actual['fused']):arrays[f'{name}_{leg}_{i}']=np.asarray(a)
            def foreign():
                try:new.evaluate()
    ...
    gates=dict(coverage=len(rows)==14,exact=all(r['exact'] for r in rows),
               ...
               L_speedup_2=all(r['times_ms']['two_hit']/r['times_ms']['fused']>=2 for r in rows if r['case']=='plate_0.5'))
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,
                source_sha256=hashlib.sha256((ROOT/'src/field_engine/mesh_fused_native_v1/field.cu').read_bytes()).hexdigest(),
                scope='Prepared immutable query grid. Every evaluation retraces geometry and recomputes mask/surface/signed EDT. Closed path keeps hits and mask on device, reads complete three arrays; open path preserves GWN/stateless EDT. Preparation and warmup excluded, two reversed orders. No full-service 2 ms claim.')
    (ROOT/'reports/field_mesh_fused.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_mesh_fused_arrays.npz',**arrays)
