...
def event(label):
    with (OUT/'field_edt_native_events.jsonl').open('a') as f:
...
def run():
    ...
    idle()
    import ctypes as ct
    library=ct.CDLL(os.environ['EDT_NATIVE_LIBRARY'])
    ...
    def squared_distance(binary):
        ...
        output=np.empty(binary.shape,dtype=np.int32)
        status=library.edt_squared(source.ctypes.data,*source.shape,output.ctypes.data)
        if status:raise RuntimeError(f'Native EDT status {status}')
        return output
    ...
    for leg in range(2):
        for name in names:
            ...
            for index, pitch in enumerate((0.125, 0.3, 0.7483641718372458)):
                pair = [(np.sqrt(value.astype(np.float64))*pitch).astype(np.float32) for value in squares]
    ...
    (OUT/'field_edt_native_v1.json').write_text(json.dumps(report, indent=2)+'\n')
    np.savez_compressed(OUT/'field_edt_native_v1_arrays.npz', **arrays)
    print(json.dumps(dict(gates=gates, rows=len(rows))))
    return 0 if all(gates.values()) else 1


def main():
    if (OUT/'field_edt_native_events.jsonl').exists():
