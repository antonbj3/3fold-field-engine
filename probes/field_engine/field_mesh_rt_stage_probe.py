...
def main():
    ...
    for name,(mesh,pitch,lo,closed) in cases().items():
        ...
        legs=[]
        with PreparedMeshField(v,f,pitch,lo,winding_library=os.environ['RT_WINDING_LIBRARY'],ptx=os.environ['RT_WINDING_PTX'],edt_library=os.environ['NATIVE_EDT_LARGE_LIBRARY']) as field:
            for leg in range(2):
    ...
    gates=dict(full_repeat=all(r['legs'][0]['hashes']==r['legs'][1]['hashes'] for r in rows),
               ...
               method_selection=all(r['method']==('rt_signed' if r['closed'] else 'ordered_gwn') for r in rows),
    ...
    (folder/'mesh_rt_stage.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(folder/'mesh_rt_stage_arrays.npz',**arrays)
