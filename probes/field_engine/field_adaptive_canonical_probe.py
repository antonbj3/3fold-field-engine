"""Canonical face-order fixture beside the frozen adaptive probe, CPU only."""
...
def canonical_faces(faces):
    """Sort existing oriented index triples; preserve duplicates and winding."""
    raw = np.asarray(faces)
    if raw.ndim != 2 or raw.shape[1] != 3 or not len(raw) or not np.issubdtype(raw.dtype, np.integer):
        raise ValueError('Nonempty integral triangle records required')
    if raw.min() < 0 or int(raw.max()) > np.iinfo(np.int64).max:
        raise ValueError('Triangle index outside nonnegative int64 range')
    return np.ascontiguousarray(raw[np.lexsort((raw[:, 2], raw[:, 1], raw[:, 0]))], dtype=np.int64)


def canonical_controls():
    original = np.array([[7, 2, 3], [1, 2, 9], [1, 2, 3], [7, 2, 3]], np.int64)
    expected = np.array([[1, 2, 3], [1, 2, 9], [7, 2, 3], [7, 2, 3]], np.int64)
    frozen = original.copy()
    result = [np.array_equal(canonical_faces(original), expected), np.array_equal(original, frozen)]
    for bad in (original.astype(float), original[0], original[:0], original[:, :2], -original,
                np.full((1, 3), 2**63, np.uint64), np.ones((2, 3), bool)):
        try: canonical_faces(bad)
...
def main():
    ...
    rows, arrays = [], {}
    order_controls, preservation, original_grids = [], [], []
    for leg in range(2):
        vertices, raw_faces = baseline._bracket_mesh()
        before = raw_faces.copy()
        faces = canonical_faces(raw_faces)
        permutation = np.random.default_rng(730 + leg).permutation(len(raw_faces))
        order_controls.append(np.array_equal(faces, canonical_faces(raw_faces[permutation])))
        preservation.append(Counter(map(tuple, faces)) == Counter(map(tuple, raw_faces)) and np.array_equal(raw_faces, before))
        ...
        with np.load(ROOT/"reports/adaptive_blocks_arrays.npz") as original:
            original_grids.append(samples.tobytes() == original[f"leg_{leg}_grid"].tobytes() and samples.shape == original[f"leg_{leg}_grid"].shape)
        for key, value in (("vertices", vertices), ("faces", faces), ("points", combined), ("origin", fine["origin"]), ("pitch", pitch)):
            arrays[f"input_{leg}_{key}"] = np.asarray(value)
    ...
    gates = dict(grid_exact=all(r["grid_differences"] == 0 for r in all_rows),
                 ...
                 original_grid_exact=all(original_grids), face_permutations=all(order_controls),
                 face_records_preserved=all(preservation), canonical_controls=all(canonical_controls()),
                 input_repeat=all(arrays[f"input_0_{key}"].tobytes()==arrays[f"input_1_{key}"].tobytes() for key in ("vertices", "faces", "points", "origin", "pitch")))
    report = dict(status="VERIFIED-FRESH" if all(gates.values()) else "OWN-GATE-FAIL",
                  scope="Canonicalized synthetic face-row order before the unchanged adaptive/raster/query pipeline. Original probe and cross-host negative retained. Dense fine reference still required during build.",
                  source_sha256={name: hashlib.sha256((_field_path(ROOT, name)).read_bytes()).hexdigest() for name in ("field_adaptive_canonical_probe.py", "adaptive_sample_blocks_v1.py", "field_adaptive_blocks_probe.py")},
                  rows=rows, controls=control_rows, rejected=rejected, gates=gates)
    dest = ROOT / "reports"
    dest.mkdir(exist_ok=True)
    (dest / "adaptive_canonical.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    np.savez_compressed(dest / "adaptive_canonical_arrays.npz", **arrays)
