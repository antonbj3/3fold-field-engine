"""Native-call versus Python wrapper attribution inside the RT stage."""
...
def main():
    ...
    gates=dict(coverage=len(rows)==24,exact=all(r['exact'] for r in rows),
               ...
               accounting=all(0<=sum(v for k,v in r['parts_ms'].items() if k!='native_rt')<=r['total_ms'] for r in rows))
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,
                scope='Direct prepared stage with one untimed warmup, two reversed size orders and four timed evaluations each. Component methods are already synchronous; no extra GPU fence. native_rt is nested inside rt_query, not an additional stage. No IPC and no 2 ms acceptance claim.')
    (ROOT/'reports/field_rt_call_profile.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_rt_call_profile_arrays.npz',**arrays)
