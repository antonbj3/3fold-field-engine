"""Bounded-template paired EDT against exact squared and signed references."""
...
def main():
    old=NativeEDTLarge(os.environ['NATIVE_EDT_LARGE_LIBRARY']);library=os.environ['NATIVE_EDT_PAIR_LIBRARY']
    ...
    for name,mask in cases:
        ...
        for leg in range(2):
            ...
            try:
                ...
                times={};outputs={}
                for mode in (('old','persistent') if leg==0 else ('persistent','old')):
                    outputs[mode],times[mode]=timed(lambda:old.signed_distance(mask,.5) if mode=='old' else handle.query(mask,.5))
                exact=[]
                ...
                rows.append(dict(case=name,leg=leg,shape=mask.shape,setup_ms=setup_ms,first_query_ms=first_ms,warm_ms=times,
                                 exact=all(exact) and actual.tobytes()==outputs['old'].tobytes()==outputs['persistent'].tobytes(),
    ...
    gates=dict(coverage=len(rows)==18,exact=all(r['exact'] for r in rows),
               ...
               plate_speedup_2=all(r['warm_ms']['old']/r['warm_ms']['persistent']>=2 for r in rows if r['case']=='plate_L'))
    report=dict(status='VERIFIED-FRESH' if all(gates.values()) else 'OWN-GATE-FAIL',gates=gates,rows=rows,controls=controls,
                source_sha256=hashlib.sha256((ROOT/'src/field_engine/edt_pair_templated_v1/edt.cu').read_bytes()).hexdigest(),
                scope='Persistent buffers and paired envelope plus correctly rounded GPU signed output. Warm calls include mask validation/upload/full signed-field download. Setup and first query separate. Five pitches include contract endpoints. Full masks recomputed after change, thread/close controls. No full mesh-stage or 2 ms service claim.')
    (ROOT/'reports/field_templated_pair.json').write_text(json.dumps(report,indent=2)+'\n')
    np.savez_compressed(ROOT/'reports/field_templated_pair_arrays.npz',**arrays)
