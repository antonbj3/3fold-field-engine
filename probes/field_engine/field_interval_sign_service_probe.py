...
def main():
    libraries = dict(fused_library=os.environ['MESH_FUSED_TAIL_GRAPH_LIBRARY'],
                     winding_library=os.environ['RT_COLUMNS_LIBRARY'], ptx=os.environ['RT_INTERVAL_SIGN_PTX'],
    ...
    sources = ('mesh_field_shared_service_v1.py', 'mesh_fused_into_v1.py', 'mesh_fused_shared_v1/field.cu', 'mesh_fused_tail_graph_v1/field.cu', 'mesh_field_owned_service_v1.py',
               'rt_columns_interval_sign_v1/filter.cuh', 'rt_columns_interval_sign_v1/program.cu', 'mesh_fused_native_v1/field.cu', 'field_interval_sign_service_probe.py')
    ...
    (ROOT/'reports/field_interval_sign_service.json').write_text(json.dumps(report, indent=2) + '\n')
    np.savez_compressed(ROOT/'reports/field_interval_sign_service_arrays.npz', **captures)
