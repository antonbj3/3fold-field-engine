"""Evaluate ordered CSG with explicit native CPU tile reading and packing.
...
def evaluate(fields, instructions, *, reader, pack_library):
    ...
    for lower in itertools.product(*(range(0,n,t) for n,t in zip(first.shape,tile_shape))):
        ...
        values = {i: reader.gather(fields[i],lower,upper) for i in used_inputs}
        for number, (operation,left,right) in enumerate(program):
            a = values[left]
...
if __name__ == '__main__':
    ...
    from field_csg_packed_probe import main
