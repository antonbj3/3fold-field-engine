// Bounded synchronous exact squared EDT. Host pointers are caller-owned.
...
extern "C" int edt_squared(const uint8_t* mask,int nx,int ny,int nz,int32_t* output) {
    if(!mask || !output || nx<1 || ny<1 || nz<1 || nx>128 || ny>128 || nz>128) return 1;
    int size=nx*ny*nz;
    if(size>262144) return 1;
    int ones=0;
