...
struct State {
    ...
    CUdeviceptr interval_corners{},interval_queries{},boxes{},queries{},depths{},signs{},vertices{},scratch{},storage{},sbt_data{},ray_data{},output{},params_data{};
    ...
    void initialize(const std::string& ptx,const std::vector<double3>& corners) {
        ...
        std::vector<float4> prepared(corners.size());
        for(size_t i=0;i<corners.size();++i)prepared[i]=host_interval_point(corners[i].x,corners[i].y,anchor);
        interval_corners=allocate(prepared.size()*sizeof(float4),prepared.data());
        ...
        descriptions[2].hitgroup.entryFunctionNameIS = "__intersection__prepared_columns";
        ...
        interval_queries=allocate(size_t(capacity)*sizeof(float4));
        ...
        parameters={gas,reinterpret_cast<double3*>(vertices),reinterpret_cast<float3*>(ray_data),
          ...
          reinterpret_cast<int*>(signs),hit_capacity,area_floor,reinterpret_cast<float4*>(interval_corners),reinterpret_cast<float4*>(interval_queries)};
    ...
    int release() noexcept {
        ...
        for(auto ptr:{interval_corners,interval_queries,boxes,queries,depths,signs,params_data,output,ray_data,sbt_data,scratch,storage,vertices})
...
static void upload_interval_queries(State& s,const double* xy,uint32_t count){
    std::vector<float4> prepared(count);
    for(uint32_t i=0;i<count;++i)prepared[i]=host_interval_point(xy[2*i],xy[2*i+1],s.anchor);
    check(cudaMemcpy(reinterpret_cast<void*>(s.interval_queries),prepared.data(),size_t(count)*sizeof(float4),cudaMemcpyHostToDevice));
}
extern "C" int columns_create(const char* path,const double* input,uint32_t triangles,uint32_t capacity,uint32_t hit_capacity,const double* anchor,void** handle) {
    ...
    State* state=nullptr;
    try {
        std::ifstream file(path);std::string ptx{std::istreambuf_iterator<char>(file),{}};if(ptx.empty()||ptx.find("__intersection__prepared_columns")==std::string::npos)return 1;
...
extern "C" int columns_query(void* handle,const double* xy,uint32_t count,int32_t* counts,double* depths,int32_t* signs) {
    ...
    try{
        ...
        upload_interval_queries(s,xy,count);
