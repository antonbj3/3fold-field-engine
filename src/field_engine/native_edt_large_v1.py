...
def validate(mask):
    ...
    if mask.ndim!=3 or min(mask.shape)<1 or max(mask.shape)>512 or mask.size>1048576:
        raise ValueError('Expected bounded larger 3D shape')
    if not mask.any() or mask.all():
        raise ValueError('Both classes must be present')


class NativeEDTLarge:
    """One host thread, externally coordinated CUDA runtime, bounded mixed masks."""
    def __init__(self, library):
        self._thread=threading.get_ident()
        self._failed=False
        self._library=ct.CDLL(str(library))
        self._library.edt_squared_large.argtypes=[ct.c_void_p,ct.c_int,ct.c_int,ct.c_int,ct.c_void_p]
        self._library.edt_squared_large.restype=ct.c_int

    def _check(self):
        if threading.get_ident()!=self._thread:
            raise RuntimeError('NativeEDTLarge belongs to another thread')
        if self._failed:
            raise RuntimeError('NativeEDTLarge runtime previously failed; no retry')
    ...
    def squared_distance(self, binary):
        ...
        status=self._library.edt_squared_large(source.ctypes.data,*source.shape,output.ctypes.data)
        if status:
...
if __name__=='__main__':
    ...
    from field_edt_large_probe import main
