// Same paired EDT with bounded 32/128/256/512 local-array specializations.
...
__global__ void axis_envelope(const int* source,int* target,int size,int stride,int length) {
    ...
    int sites[Bound],starts[Bound+1];int k=0;
...
static void launch_axis_pair(const int* source,int* target,int size,int stride,int length) {
    int blocks=(2*size/length+127)/128;
    if(length<=32)axis_envelope<32><<<blocks,128>>>(source,target,size,stride,length);
    else if(length<=128)axis_envelope<128><<<blocks,128>>>(source,target,size,stride,length);
    else if(length<=256)axis_envelope<256><<<blocks,128>>>(source,target,size,stride,length);
    else axis_envelope<512><<<blocks,128>>>(source,target,size,stride,length);
}
...
extern "C" int edt_pair_query(void* pointer,const uint8_t* mask,double pitch,float* output,int32_t* squared) noexcept {
    ...
    for(int axis=0;axis<3;++axis){
        launch_axis_pair(a,b,p->size,strides[axis],lengths[axis]);
