"""Device-resident closed-mesh RT/mask/EDT, with frozen open-mesh GWN fallback.
...
from mesh_fused_handle_v1 import FusedField
from column_mask_native_v1 import NativeColumnMask
from native_edt_large_v1 import NativeEDTLarge


class PreparedMeshField:
    def __init__(self,vertices,faces,pitch,lo,*,fused_library,winding_library,ptx,edt_library,column_library,global_shape=None,margin_vox_pad=2):
        ...
        if self.method=='rt_columns':
            ...
            self._rt=FusedField(fused_library,ptx,corners,anchor,self._xy,shape,self._pitch)
    ...
    def evaluate(self):
        ...
        try:
            if self.method=='rt_columns':
                solid,surface,distance=self._rt.query()
            else:
