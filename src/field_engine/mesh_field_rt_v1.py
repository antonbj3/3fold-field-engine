"""Prepared closed-mesh RT or open-mesh GWN, followed by exact native EDT.
...
class PreparedMeshField:
    def __init__(self,vertices,faces,pitch,lo,*,winding_library,ptx,edt_library,global_shape=None,margin_vox_pad=2):
        ...
        self.method='rt_signed' if mesh.is_watertight and mesh.is_winding_consistent else 'ordered_gwn'
        self._vertices=v-anchor;self._faces=t.astype(np.int64);self._pitch=float(pitch)
        ...
        if self.method=='rt_signed':
            corners=np.ascontiguousarray(self._vertices[self._faces],dtype=np.float32)
            self._rays=np.ascontiguousarray(self._points,dtype=np.float32);self._rays.flags.writeable=False
            self._rt=WindingHandle(winding_library,ptx,corners,len(self._rays))
    ...
    def evaluate(self):
        self._check()
        try:
            if self.method=='rt_signed':solid=self._rt.query(self._rays).reshape(self.shape)!=0
            else:solid=(np.abs(generalized_winding(self._vertices,self._faces,self._points))>.5).reshape(self.shape)
...
def surface_raster_and_flood_rt(vertices,faces,pitch,lo,*,winding_library,ptx,edt_library,global_shape=None,margin_vox_pad=2):
    """One-shot stage including preparation/close; no hidden global cache."""
    with PreparedMeshField(vertices,faces,pitch,lo,winding_library=winding_library,ptx=ptx,edt_library=edt_library,
...
if __name__=='__main__':
    ...
    from field_mesh_rt_stage_probe import main
