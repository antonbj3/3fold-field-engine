// Exact bit copies from packed adaptive leaves into one bounded tile.
#include <cstdint>
#include <cstring>
#include <algorithm>
#include <vector>
    ...
    try {
        ...
        for(int64_t n=0;n<count;++n) {
            ...
            for(int x=lo[0];x<hi[0];++x) for(int y=lo[1];y<hi[1];++y) for(int z=lo[2];z<hi[2];++z) {
                ...
                source+=node.stored[2]==1?0:z-node.lo[2];
                int64_t target=((x-lower[0])*dims[1]+y-lower[1])*dims[2]+z-lower[2];
                if(coverage[target]) return 4;
                coverage[target]=1;
                std::memcpy(static_cast<char*>(output)+4*target,static_cast<const char*>(payload)+4*source,4);
            }
        }
