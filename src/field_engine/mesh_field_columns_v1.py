"""Prepared closed-mesh exact RT columns or open-mesh GWN, followed by exact native EDT.
...
from rt_columns_handle_v1 import ColumnsHandle, occupancy
...
class PreparedMeshField:
    def __init__(self,vertices,faces,pitch,lo,*,winding_library,ptx,edt_library,global_shape=None,margin_vox_pad=2):
        ...
        self.method='rt_columns' if mesh.is_watertight and mesh.is_winding_consistent else 'ordered_gwn'
        ...
        if self.method=='rt_columns':
            ...
            self._rt=ColumnsHandle(winding_library,ptx,corners,anchor,len(self._xy))
    ...
    def evaluate(self):
        self._check()
        try:
            if self.method=='rt_columns':solid=occupancy(*self._rt.query(self._xy),self._pitch,self._anchor,self.shape)
...
if __name__=='__main__':
    ...
    from field_mesh_columns_stage_probe import main
