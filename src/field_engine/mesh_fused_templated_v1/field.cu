// Same device-resident field pipeline with bounded EDT local-array templates.
#include "../rt_columns_v1/api.cpp"
#include "../edt_pair_templated_v1/edt.cu"
...
extern "C" int fused_query(void* handle,uint8_t* solid,uint8_t* surface,float* distance) noexcept {
    ...
    try{
        ...
        for(int axis=0;axis<3;++axis){launch_axis_pair(a,b,p.size,strides[axis],lengths[axis]);check(cudaGetLastError());std::swap(a,b);}
