"""Exact composite fractions on CPU, bounded synchronized native mask/surface."""
import ctypes as ct
import operator
import threading
import numpy as np
...
class NativeColumnMask:
    ...
    def evaluate(self,c,z,s,pitch,origin,shape):
        ...
        order=np.lexsort((z,c));c=c[order];z=z[order].astype(np.float64);signs=np.ascontiguousarray(s[order],np.int32)
...
if __name__=='__main__':
    ...
    from field_column_mask_probe import main
